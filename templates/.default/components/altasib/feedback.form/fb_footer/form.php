<div class="container">
	...
	<p class="mb-50 question-article">Остались вопросы?</p>
	...
	<form id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="anyquestions">
	<div class="row">
...
<?				$arrFields_userconsent[] = $arField['NAME'];
				...
				if ($arField["REQUIRED"]=='Y')
					$fieldClass .= ' required';
				if ($arField["CODE"] == "EMAIL_".$ALX || $arField["CODE"] == "FIO_".$ALX || $arField["CODE"] == "PHONE_".$ALX)
					$fieldClass .= 'col-md-4';
				else
...
<?						if($arField["LIST_TYPE"] == "L"): /*list*/?>
							<div class="afbf_inputtext_bg test">
...
<?							if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext form-control" readonly="readonly" onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							else:?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext form-control" readonly="readonly" onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							endif;?>
...
<?				elseif($arField["TYPE"] != "F"):
					$placeholder = '';
					if($arField["CODE"] == 'PHONE_FID1')
						$placeholder = 'placeholder="Номер телефона"';
					if($arField["CODE"] == 'FIO_FID1')
						$placeholder = 'placeholder="Имя"';
					if($arField["CODE"] == 'EMAIL_FID1')
						$placeholder = 'placeholder="E-mail"';
?>
					<div class="afbf_inputtext_bg" id="error_<?=$arField["CODE"]?>">
<?						if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext form-control" <?=$placeholder?> />
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							$readonly = "";
							...
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" class="afbf_inputtext form-control" <?=$placeholder?> />
<?						else:?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext form-control" <?=$placeholder?> onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" />
<?						endif;?>
...
<?					endif;?>
						<div class="afbf_input_group">
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[myFile][<?=$arField["CODE"]?>]" value="" class="afbf_inputtext form-control" />
...
<?
			...
			$textAreaID = 'EMPTY_TEXT'.$ALX;
			$textAreaName = 'FEEDBACK_TEXT_'.$ALX;
?>
			<div class="col-md-12">
					<textarea  class="form-control" placeholder="Что вас интересует?" rows="5" id="<?=$textAreaID?>" name="<?=$textAreaName?>"></textarea>
			</div>
...
<?						if($fVerComposite) $frame->end();?>
						...
						<div class="afbf_inputtext_bg"><input type="text" class="afbf_inputtext form-control" id="captcha_word_<?=$ALX?>" name="captcha_word" size="30" maxlength="50" value=""></div>
						<div class="afbf_error_text"><?=GetMessage("ALX_CP_WRONG_CAPTCHA")?></div>
					</div>
<?				else:?>
<?					if (isset($arResult["SITE_KEY"])):?>
						<div class="col-md-10 col-md-offset-1 required is_filled">
...
<?		echo bitrix_sessid_post()?>
		<div class="col-md-10 col-md-offset-1">
			<div class="checkbox personal">
					...
					<label for="check1">Я согласен на обработку <a href="confidence.html">персональных данных</a></label>
			</div>
		</div>
		<div class="col-md-10 col-md-offset-1">
				<input type="submit" class="btn btn-square mt-10 mr-30 mb-30" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="Отправить">
		</div>
	</div>
	</form>
