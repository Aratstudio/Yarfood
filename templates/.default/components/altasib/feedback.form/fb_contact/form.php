<div>
	...
	<?endif;?>
	<form class="contact-form mb-50 row" id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
...
<?			if((is_array($arParams["PROPERTY_FIELDS"]) && in_array("FEEDBACK_TEXT", $arParams["PROPERTY_FIELDS"]))
			...
			)
			{
				...
				$strFBtext = '<div class="col-12 mb-25';
				...
				$strFBtext .= '</div>
					<div class="afbf_inputtext_bg" id="error_EMPTY_TEXT">
						<textarea cols="10" rows="10" id="EMPTY_TEXT'.$ALX.'" name="FEEDBACK_TEXT_'.$ALX.'">'.$arResult["FEEDBACK_TEXT"].'</textarea>
					</div>
					<div class="afbf_error_text">'.GetMessage("AFBF_ERROR_TEXT").'</div>
				</div>';
...
?>
				<div id="afbf_<?=mb_strtolower($arField["CODE"])?>" class="col-md-6 col-12 mb-25 <?=$fieldClass?>">
...
?>
					<div class="afbf_inputtext_bg" id="error_<?=$arField["CODE"]?>">
<?						if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" />
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							$readonly = "";
							...
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]))?>" />
<?						else:?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" />
<?						endif;?>
...
<?					if (isset($arResult["SITE_KEY"])):?>
						<div class="col-12 required is_filled">
...
<?		echo bitrix_sessid_post()?>
		...
		<div class="col-12 mb-25">
			<input type="submit" class="" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="<?=GetMessage('ALX_TP_MESSAGE_SUBMIT')?>" />
		</div>
	</form>
