...
//удаляем разделы, которые помечены UF_HIDE
foreach ($arResult['SECTIONS'] as $key => $arSection) {
	$parentHide;
	$arFilter = Array('IBLOCK_ID'=>$arSection['IBLOCK_ID'], 'GLOBAL_ACTIVE'=>'Y', "ID"=>$arSection['ID']);
	$db_list = CIBlockSection::GetList(Array($by=>$order), $arFilter, true, Array('UF_HIDE'));
...
//		d($ar_result);
	   ...
	   if($ar_result["UF_HIDE"] == 1) {
		   $parentHide = $ar_result["ID"];
		   unset($arResult['SECTIONS'][$key]);
	   }
		 
		 if($parentHide != "") {
			 if($arSection["IBLOCK_SECTION_ID"] == $parentHide) {
			   unset($arResult['SECTIONS'][$key]);
		   }
	   }
