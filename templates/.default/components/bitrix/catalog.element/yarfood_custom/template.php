...
if($arSection = $rsSection->GetNext())
	if(!empty($arSection['UF_ALT_TITLE']))
		$arResult['SECTION']['NAME'] = $arSection['UF_ALT_TITLE'];
...
<div class="product-section section mt-90 mb-90" id="<?=$itemIds['ID']?>"
            ...
            <div class="col-lg-7 col-12 mb-50">
                ...
                <div class="single-product-content">
                    ...
                    <div class="head-content">
                        ...
                        <h5 class="price"><?=$arResult['MIN_PRICE']['PRINT_VALUE']?></h5>
                    ...
                    if($arResult['CAN_BUY'] ? 'В наличии' : 'Нет в наличии');
                        if($arResult['CATALOG_QUANTITY']==0){
                 
                            echo('<span class="must-order">Под заказ</span>');
                        }
                        else{echo('<span class="must-available">В наличии</span>');
						...
						<? if($arResult['CATALOG_QUANTITY']===0){
echo('<p class="non-availabe">Данный товар отсутствует на складе в Красноярске. Доставка под заказ в течение 7 - 14 дней</p>');
                        }else{echo('');}?>
                        ...
                        <div class="actions">
							<div data-entity="main-button-container">
								<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>">
									<a id="<?=$itemIds['ADD_BASKET_LINK']?>" href="javascript:void(0);" class="add-to-cart"><i class="ti-shopping-cart"></i><span>ЗАКАЗАТЬ</span></a>
								</div>
							</div>

                            <div class="wishlist-compare">
                                <a id="<?=$itemIds['COMPARE_LINK']?>" class="<?=($arResult['IS_COMPARED_ITEM'] == 'Y' ? 'added' : '')?>" href="#" data-tooltip="Сравнить"><i class="ti-control-shuffle"></i></a>
                ...
                <div class="single-product-tab-content tab-content">
                    <div class="tab-pane fade show active" id="product-description">
                        ...
                        <div class="row">
							...
							<div class="text-more" tabindex="0">
<?=$arResult['DETAIL_TEXT']?>

									<p class="textmore">Показать еще...</p>
                            </div>
                            </div>
							<!--
                    ...
                    <div class="tab-pane fade" id="product-specifications">
						...
						<? if($prop['NAME']==="Реквизиты"){
                                    unset($prop['NAME']);unset($prop['VALUE']);
                                  
							}?>

<? if($prop['NAME']==="Лидер продаж"){
                                    unset($prop['NAME']);unset($prop['VALUE']);

							}?>
<? if($prop['NAME']==="Рекомендуемые товары"){
                                    unset($prop['NAME']);unset($prop['VALUE']);
...
<? if($prop['NAME']==="Ширина ванны мм."){
   unset($prop['NAME']);unset($prop['VALUE']);

}?><!-- -->

						<? if($prop['NAME']==="Ставки налогов"){
									unset($prop['NAME']);unset($prop['VALUE']);
						}?>	
                        <? if($prop['NAME']==="Температурный режим min, °С"){?>
                            <?if($prop['VALUE']==="0"){?>
                                <br/><li><?print_r($prop['NAME']);echo(" — ");print_r($prop['VALUE']);?>
                            <?}?>
						...
						<?if(!empty($prop['VALUE'])){?>
							...
							<li><?print_r($prop['NAME']);echo(" — ");print_r($prop['VALUE']);?>
						<?} ?>
						<?endforeach;?>
						</ul>
                        </div>
...
<div class="product-section section mb-70">
    <div class="container">
        <div class="row">
            ...
            <div class="col-12 mb-40">
                <div class="section-title-one" data-title="FEATURED ITEMS"><h1>Рекомендуем</h1></div>
            </div><!-- Section Title End -->
            
            <!-- Product Tab Filter Start -->
            <div class="col-12 mb-30">
                <div class="product-tab-filter"  style="display: none;">
                    <!--
                    <button class="product-tab-filter-toggle">showing: <i class="icofont icofont-simple-down"></i></button>

                    <ul class="nav product-tab-list">
                        <li><a class="active" data-toggle="tab" href="#tab-one">все</a></li>
                        <li><a data-toggle="tab" href="#tab-two">ПРОФ ХИМИЯ</a></li>
                        <li><a data-toggle="tab" href="#tab-one">ЗАПЧАСТИ</a></li>
                        <li><a data-toggle="tab" href="#tab-two">ОБОРУДОВАНИЕ</a></li>
                    </ul>
                    -->
                </div>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade show active" id="tab-one">
										...
										if (!is_array($arrFilter))
										  $arrFilter = array();
											$arrFilter['ID'] = $arResult["PROPERTIES"]["RECOMMEND"]["VALUE"];   // ID нужных элементов
										?>
	...
	array(
		...
		"COMPATIBLE_MODE" => "Y",
		"CONVERT_CURRENCY" => "N",
		//"CUSTOM_FILTER" => "{\"CLASS_ID\":\"CondGroup\",\"DATA\":{\"All\":\"AND\",\"True\":\"True\"},\"CHILDREN\":[{\"CLASS_ID\":\"CondIBProp:58:19430\",\"DATA\":{\"logic\":\"Equal\",\"value\":27061}}]}",
		"DETAIL_URL" => "",
		"DISPLAY_COMPARE" => "Y",
		"ELEMENT_COUNT" => "8",
		...
		"USE_PRODUCT_QUANTITY" => "N",
		"VIEW_MODE" => "SECTION",
		"COMPONENT_TEMPLATE" => "xliders",
		"COMPARE_PATH" => "/category/compare.php",
		"ROTATE_TIMER" => "30",
		"SHOW_PAGINATION" => "Y",
...
<?$APPLICATION->IncludeComponent(
	...
	array(
		...
		"COMPATIBLE_MODE" => "Y",
		"CONVERT_CURRENCY" => "N",
		"CUSTOM_FILTER" => "{\"CLASS_ID\":\"CondGroup\",\"DATA\":{\"All\":\"AND\",\"True\":\"True\"},\"CHILDREN\":[{\"CLASS_ID\":\"CondIBProp:58:19989\",\"DATA\":{\"logic\":\"Equal\",\"value\":28872}}]}",
		"DETAIL_URL" => "",
		"DISPLAY_COMPARE" => "Y",
		"ELEMENT_COUNT" => "8",
		"ELEMENT_SORT_FIELD" => "sort",
...
if(isset($arResult['PROPERTIES']['RELATED_ITEM']) && !empty($arResult['PROPERTIES']['RELATED_ITEM'])) {
	$GLOBALS['arrProductsFilter'] = array(
		'ID' => $arResult['PROPERTIES']['RELATED_ITEM']['VALUE']
	);
	$APPLICATION->IncludeComponent(
		"bitrix:catalog.top",
		"accessory",
...
<script>
	//смена пункта параметров на телефоне
	if($(window).width() < 768) {
		let menuItem = document.querySelectorAll('.single-product-tab-list.nav li a');
		let menuBlock = document.querySelectorAll('.single-product-tab-content.tab-content .tab-pane');

		console.log(menuItem);
		menuItem[0].setAttribute("class", "nonactive");
		menuItem[1].setAttribute("class", "active");
		menuBlock[0].setAttribute("class", "tab-pane fade");
		menuBlock[1].setAttribute("class", "tab-pane fade show active");
	}
	</script>