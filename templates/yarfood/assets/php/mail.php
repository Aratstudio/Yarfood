<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ( empty($first_name) OR empty($last_name) OR empty($phone) OR empty($message) OR !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            echo "Пожалуйста, заполните необходимые поля и повторите отправку письма.";
            exit;
        }
        ...
        if (mail($recipient, $subject, $email_content, $email_headers)) {
            ...
            echo "Ваше сообщение успешно отправлено! Наши спициалисты свяжутся с вами в ближайшее время.";
        } else {
            // Set a 500 (internal server error) response code.
            http_response_code(500);
            echo "Что-то пошло не так. Ошибка отправки сообщения";
        }
